  ...
  return (
    <section id="galeri">
      <header className="wrapper">
        <h1 className="pb-4 font-bold text-5xl w-full text-center text-dark-gray dark:text-dark-white">
          Galeri
        </h1>
        <p className="desc text-light-teal/60 dark:text-gray-400 italic text-center">
          Dükkan Fotoraf Galerimiz
        </p>
      </header>
      <main className={`${style.gallery} wrapper`}>
        <LightGallery speed={500} plugins={[lgThumbnail, lgZoom]}>
          {photoItems.map(({ href, img }, i) => (
            <a key={i} href={href}>
              <Image {...img} width={500} height={500} loading="lazy" placeholder="empty" />
            </a>
          ))}
        </LightGallery>
      </main>
